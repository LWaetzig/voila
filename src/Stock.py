...
class Stock():
    """Class to load and clean up stock data from yahoo finance 
    """
    ...
    def get_stock_data(self) -> pd.DataFrame:
        ...
        data["mva50"] = data["Open"].rolling(50).mean()
        data["mva200"] = data["Open"].rolling(200).mean()
    ...
    def create_future_dates(self , df) -> pd.DataFrame:
...
class Predictor(Stock):
    ...
    def make_prediction(self , df) -> pd.DataFrame:
        ...
        if self.model == "linreg":
            pred_model = np.poly1d(np.polyfit(df["date_value"], df["Close"], deg=self.deg))

            super().__init__
            future_dates_df = Stock.create_future_dates(df)

            pred_df = pd.DataFrame(columns="predicted")
            pred_df["predicted"] = pred_model(future_dates_df["date_values"])
