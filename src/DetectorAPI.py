...
class Detector:
    def __init__(self, model_path, name=""):
        ...
        logging.info(f"{self.model_name} model is created.")

    def detect_objects(self, img, threshold):
        ...
        logging.info("{} : Object detection has started.".format(self.model_name))
        ...
        with tf.compat.v1.Session(graph=self.graph) as sess:
            ...
            # run the model
            (num, scores, boxes, classes) = self.sess.run(
                [
            ...
            logging.info(f"{self.model_name} : {len(objects)} objects have been found ")
        end_time = time.time()
        logging.info(
            "{} : Elapsed time: {}".format(self.model_name, str(end_time - start_time))
        )
